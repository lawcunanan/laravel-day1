                    ...
                    @foreach ($tasks as $task)
                        <tr class="border-b border-green-200 hover:bg-green-100 transition">
                            ...
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('editTask.edit', $task) }}" class="bg-green-500 hover:bg-green-600 text-white font-semibold py-1 px-3 rounded-lg shadow-sm transition">Edit</a>
                                <form action="{{ route('updateTasks.status', $task) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button class="bg-red-500 hover:bg-red-600 text-white font-semibold py-1 px-3 rounded-lg ml-2 shadow-sm transition">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
